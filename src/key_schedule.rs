...
impl KeySchedule {
  ...
  pub fn sign_verify_data(&self, kind: SecretKind, hs_hash: &[u8]) -> Vec<u8> {
    ...
    hmac::sign(
      &hmac::SigningKey::new(self.hash, &hmac_key),
      hs_hash
    ).as_ref().to_vec()
  }
}
...
pub fn hkdf_expand_label(hash: &'static digest::Algorithm,
                         ...
                         context: &[u8],
                         len: u16) -> Vec<u8> {
  _hkdf_expand_label(&hmac::SigningKey::new(hash, secret),
                     label,
                     context,
                     len)
}
...
mod test {
  ...
  fn smoke_test() {
    ...
    ks.derive(SecretKind::ClientHandshakeTrafficSecret, &fake_handshake_hash);
    ks.derive(SecretKind::ServerHandshakeTrafficSecret, &fake_handshake_hash);
    ks.input_empty();
    ks.derive(SecretKind::ClientApplicationTrafficSecret, &fake_handshake_hash);
    ks.derive(SecretKind::ServerApplicationTrafficSecret, &fake_handshake_hash);
