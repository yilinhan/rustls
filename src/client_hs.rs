...
fn process_alpn_protocol(sess: &mut ClientSessionImpl, proto: Option<String>) -> Result<(), TLSError> {
...
fn handle_server_hello(sess: &mut ClientSessionImpl, m: Message) -> Result<ConnState, TLSError> {
  ...
  match server_hello.server_version {
    ...
    ProtocolVersion::TLSv1_3 | ProtocolVersion::Unknown(TLS13_DRAFT)
      if sess.config.versions.contains(&ProtocolVersion::TLSv1_3) => {
...
fn handle_certificate_or_cert_req(sess: &mut ClientSessionImpl, m: Message) -> Result<ConnState, TLSError> {
...
fn handle_key_update(sess: &mut ClientSessionImpl, m: Message) -> Result<(), TLSError> {
  ...
  let new_read_key = sess.common.get_key_schedule()
