...
fn curve_nistp256() {
  ...
  server.client()
    .verbose()
    .expect_log("ECDHE curve is ECParameters { curve_type: NamedCurve, named_group: secp256r1 }")
    .go();
  server.kill();
}
...
fn curve_nistp384() {
  ...
  server.client()
    .verbose()
    .expect_log("ECDHE curve is ECParameters { curve_type: NamedCurve, named_group: secp384r1 }")
    .go();
  server.kill();
}
